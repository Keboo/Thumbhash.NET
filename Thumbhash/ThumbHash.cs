...
public static class ThumbHash
{
    ...
    public static byte[] RgbaToThumbHash(int w, int h, byte[] rgba)
    {
        ...
        int lx = Math.Max(1, (int)Math.Round((float)(l_limit * w) / (float)Math.Max(w, h)));
        int ly = Math.Max(1, (int)Math.Round((float)(l_limit * h) / (float)Math.Max(w, h)));
        ...
        int header24 = (int)Math.Round(63.0f * l_channel.dc)
                | ((int)Math.Round(31.5f + 31.5f * p_channel.dc) << 6)
                | ((int)Math.Round(31.5f + 31.5f * q_channel.dc) << 12)
                | ((int)Math.Round(31.0f * l_channel.scale) << 18)
                | (hasAlpha ? 1 << 23 : 0);
        int header16 = (isLandscape ? ly : lx)
                | ((int)Math.Round(63.0f * p_channel.scale) << 3)
                | ((int)Math.Round(63.0f * q_channel.scale) << 9)
                | (isLandscape ? 1 << 15 : 0);
        ...
        if (hasAlpha) hash[5] = (byte)((byte)Math.Round(15.0f * a_channel.dc)
    ...
    private class Channel
    {
        ...
        public int writeTo(byte[] hash, int start, int index)
        {
            ...
            {
                hash[start + (index >> 1)] |= (byte)((int)Math.Round(15.0f * v) << ((index & 1) << 2));
                index++;
            }
            return index;
        }
